# s54_al_ablation.py — Fine-tuning strategy ablation study
import torch
import numpy as np
...
from utils.metrics import (count_model_parameters, calculate_inference_speed, save_pr_data,
                           ...
                           compute_detailed_metrics, save_detailed_results)
from utils.cache import get_cache_key, save_strategy_state, load_strategy_state
from param_config import config, training_args_, ft_str, is_random
...
def setup_model(model_path, num_labels):
    ...
    """Initialize model and tokenizer, select model architecture based on config"""
    print(f"Setting up model with {num_labels} labels...")

    # Get base model type from config
    model_type = baseModel.lower()
    print(f"Using model type: {model_type}")

    # Select corresponding class based on model type
    if model_type == 'albert':
        ...
        tokenizer = MobileBertTokenizer.from_pretrained(model_path, local_files_only=True)
        ...
        raise ValueError(f"Unsupported model type: {model_type}. Supported types: "
                        f"albert, bert, roberta, tinybert, mobilebert, distilbert")
...
def data_collator(data):
    """Data collation function for batching"""
    return {
...
def train_model(model, train_dataset, training_args):
    ...
    try:
        ...
        # Disable mixed precision, use standard precision
        ...
        optimizer = torch.optim.AdamW(
            ...
            weight_decay=0.01,
            eps=1e-8,
            betas=(0.9, 0.999)  # Default values, good general settings
        )
...
def main():
    ...
    try:
        ...
        try:
            ...
            # Determine processing method based on system resources
            ...
            # X, y = balanced_sample_data(X, y, samples_per_class=200)
            # print(f"Balanced dataset size: {len(y)}")
        ...
        if cached_state is not None:
            ...
            # If no cache, run normal initialization
        ...
        # 9. Active learning iteration
        iteration = 0
        save_interval = 10  # Set save interval to 10 iterations
        ...
        while iteration < config.al.max_iterations:
            ...
            try:
                ...
                train_dataset = TextDataset(
                    ...
                    tokenizer=tokenizer,
                    num_labels=num_labels  # Pass pre-computed num_labels
                )
                ...
                # Evaluate model - only evaluate at specific intervals
                ...
                if should_evaluate:
                    ...
                    # Ensure result list lengths are consistent, add placeholders
                ...
                if remaining_samples < config.al.query_size:
                    ...
                    # Compute and save final evaluation metrics (if not yet evaluated in current iteration)
                    ...
                    # Save detailed results of current state
