...
class TransformerEncoder(nn.Module):
    def __init__(self, input_dim, d_model, nhead, num_layers, max_len=256):
...
def feature_selection():
    selected_features = [
        'init_win_bytes_forward', 'init_win_bytes_backward', 'packet_length_mean',
        'avg_segment_size', 'packet_length_max', 'total_length', 'packet_length_min',
        'flow_packets_s', 'subflow_bytes', 'packet_length_std', 'packet_length_variance',
        'flow_duration', 'iat_total', 'iat_mean', 'header_length', 'iat_min', 'iat_max',
        'flow_bytes_s', 'iat_std', 'ack_flag_count', 'total_packets', 'psh_flag_count',
        'syn_flag_count', 'subflow_packets', 'idle_max'
    ]
...
def main(file_paths, output_file, batch_size, label_mapping):
    ...
    model = TransformerEncoder(input_dim=1, d_model=64, nhead=4, num_layers=2)
    model.eval()
    ...
    print("Concatenating features and mapping labels...")
    feature_cic = np.array(feature_cic)
    feature_lengths = np.array(feature_lengths)
    concatenated_features = np.hstack((feature_cic, feature_lengths))

    mapped_labels = np.array(labels)

    print(f"Saving data to {output_file}...")
    np.savez(output_file, features=concatenated_features, labels=mapped_labels)

    print("Done!")
    print(f"Feature shape: {concatenated_features.shape}")
    print(f"Label shape: {mapped_labels.shape}")
    print(f"Unique labels: {np.unique(mapped_labels)}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extract CIC features from JSON and encode to NPZ")
    parser.add_argument('files', nargs='+', help='Input JSON file paths')
    parser.add_argument('-o', '--output', default='processed_data.npz', help='Output NPZ file path')
    parser.add_argument('-b', '--batch_size', type=int, default=1000, help='Batch size for processing')
    parser.add_argument('--label_mapping_file', default=None,
                        help='Label mapping JSON file path (format: {"label_name": int_id, ...}). '
                             'If not provided, edit the label_mapping dict in the code.')
    ...
    if args.label_mapping_file:
        ...
        # Edit this mapping for your dataset, e.g. (USTC dataset):
        # label_mapping = {'Facetime': 0, 'FTP': 1, 'SMB': 2, ...}
        label_mapping = {}
        print("Warning: --label_mapping_file not provided; edit the label_mapping dict in the code")

    main(args.files, args.output, args.batch_size, label_mapping)
